  ...
  $.fn.paperfold = function (options) {
    ...
    */
    function createFold(j, topHeight, bottomHeight) {
      ...
      var top = $('<div>').css(css.top).css('height', topHeight).append($('<div>').css(css.inner).css('top', offsetTop).append(content.clone()).add(topShadow));
      var bottom = $('<div>').css(css.bottom).css('height', bottomHeight).append($('<div>').css(css.inner).css('bottom', offsetBottom).append(content.clone()).add(bottomShadow));
      $tops = $tops.add(top);
    ...
    function tick(timestamp) {
      ...
      if (targetPercentage === 0) currentPercentage = 1 - currentPercentage;
      ...
      // adjust scroll
      if (settings.stayInPlace) scrollParent.scrollTop(scrollOffset + currentPercentage * height);

      if (currentPercentage !== targetPercentage) {
        ticker(tick);
      } else {
        if (targetPercentage === 1) showContent();
      }
    }
    ...
    function animateTo(percentage) {
      ...
      if (settings.stayInPlace) scrollOffset = scrollParent.scrollTop();
